...
install:
	@cd ../installer; \
	helm upgrade -i kube-auth-manager charts/kube-auth-manager --wait \
...
uninstall:
	@cd ../installer; \
	helm uninstall kube-auth-manager --namespace=$(KUBE_NAMESPACE) || true
...
.PHONY: run
run:
	go run -mod=vendor ./cmd/kube-auth-manager run \
		--v=3 \
		--secure-port=8443 \
