...
import (
	...
	authv1alpha1 "go.bytebuilders.dev/kube-auth-manager/apis/authentication/v1alpha1"
	authcontrollers "go.bytebuilders.dev/kube-auth-manager/pkg/controllers/authentication"
...
func NewCmdManager() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:               "manager",
		Short:             "Launch kube-auth-manager",
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err = (&authcontrollers.UserReconciler{
			...
			if err = (&authcontrollers.GroupReconciler{
				Client: mgr.GetClient(),
				Scheme: mgr.GetScheme(),
			}).SetupWithManager(mgr); err != nil {
